...
## Load stan model
model <- cmdstan_model("stan_model/SSM_cyclic_MarSep_SIG5.stan")
...
for(i in 1){
  ...
  out_name <- paste0("SSM_cyclic_MarSep_SIG5_", "Lagtemp", data_list$Lag_temp, "_Laglight", data_list$Lag_light, "_LagCCA1", data_list$Lag_CCA1)
  ps <- 7
  ...
  fit <- model$sample(
    ...
    sig_figs = 4,
    output_dir = "/tmp",
    output_basename = paste0("SIG5_cyclic", i)
  )
  
  # 99% Bayesian credible intervals
  outcsv_name <- list.files("/tmp")
  outcsv_name <- outcsv_name[grep(paste0("SIG5_cyclic", i), outcsv_name)]
  tmp_csv_mu <- NULL
  tmp_csv_b_temp <- NULL
