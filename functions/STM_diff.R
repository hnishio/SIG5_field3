...
STM_diff <- function(data1, data2, gene_idx, condition_name, ps){
  ...
  out_name <- paste0(condition_name, "_", names(data1)[gene_idx])
  ...
  # Calculate quantiles
  df_mu1 <- as.data.frame(t(apply(tmp_csv_mu1, 2, quantile99)))
  df_mu2 <- as.data.frame(t(apply(tmp_csv_mu2, 2, quantile99)))
  ...
  df <- cbind(data.frame(time = unique(data1$Time)),
              data.frame(data_Mar = tapply(data1[,gene_idx], data1$Time, mean),
                         data_Sep = tapply(data2[,gene_idx], data2$Time, mean)),
  ...
  g <- bayesplot::mcmc_combo(
    ...
    widths = c(1, 1),
    pars = c("mu1[1]", paste0("mu1[", data_list$N_time, "]"), "diff[1]", paste0("diff[", data_list$N_time, "]"), max_rhat),
